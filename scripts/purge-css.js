...
async function purgeUnusedCSS() {
  ...
  purgeCSSResult.forEach((result, index) => {
    ...
    console.log(
      `✅ ${originalFiles[index]}: ${originalSize} bytes → ${purgedSize} bytes (${reduction}% reduction)`
    )
  })

  console.log('🎉 CSS purging completed! Check src/assets/css/ directory')
}
